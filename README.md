...
$$ \underline{\dot{x}}=\left[\lbrack \matrix{\dot{N}_p \cr \dot{N}_a \cr \dot{d}_c \cr \dot{L}_g} \right\rbrack]=\left[\lbrack \matrix{n_p \dot{V}-\frac{N_A}{M_p}\left(\frac{N_p}{N_p+N_a}\right) \dot{m} \cr -\frac{N_A}{M_p}\left(\frac{N_p}{N_p+N_a}\right) \dot{m} \cr 2 r \cr (\text { e_(inhib) }-2) r}\right\rbrack] $$ <br />
